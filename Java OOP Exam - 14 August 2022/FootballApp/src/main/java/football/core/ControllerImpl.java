...
public class ControllerImpl implements Controller {
    ...
    public String supplementForField(String fieldName, String supplementType) {
        ...
        if (supplement1 == null) {
            ...
            Field field = fields.stream().filter(f -> f.getName().equals(fieldName))
                    .findFirst().orElseThrow(() -> new NullPointerException("Field cannot be null"));
    ...
    public String addPlayer(String fieldName, String playerType, String playerName, String nationality, int strength) {
        ...
        Field field = fields.stream().filter(f -> f.getName().equals(fieldName))
                .findFirst().orElseThrow(() -> new NullPointerException("Field cannot be null"));
        String result = null;
        if (field.getClass().getSimpleName().equals("ArtificialTurf") && field.getPlayers().size() == 150) {
            result = NOT_ENOUGH_CAPACITY;
        } else if (field.getClass().getSimpleName().equals("NaturalGrass") && field.getPlayers().size() == 250) {
            result = NOT_ENOUGH_CAPACITY;
